...
export default {
  ...
  methods: {
    ...
    //获取所有管理员列表
    getAdmin() {
      this.axios.get("/api/admin/getAdmin", {}).then(res => {
        if (res.status == 200) {
    ...
    addAdmin() {
      ...
      var name = this.manger.name;
      var passwd = this.crypto(this.manger.passwd);
      // userObj.password = this.crypto(userObj.password);
      var role = this.manger.role;
      if (name == undefined || passwd == undefined || role == undefined) {
